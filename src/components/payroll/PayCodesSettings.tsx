...
import { Plus, Search, Settings, Info, Upload, BarChart3 } from 'lucide-react';
...
import { PayCodeUsageReport } from './PayCodeUsageReport';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function PayCodesSettings() {
  ...
  // Filter pay codes based on category for different tabs
  const primaryPayCodes = payCodes.filter(pc => 
    ['earning', 'overtime', 'pto'].includes(pc.category)
  );
  
  const supplementalPayCodes = payCodes.filter(pc => 
    ['premium', 'bank', 'deduction', 'benefit'].includes(pc.category)
  );

  const getFilteredPayCodes = (codes: PayCode[]) => {
    return codes.filter(payCode => {
  ...
  const renderPayCodesTab = (codes: PayCode[], title: string, description: string) => {
    const filteredCodes = getFilteredPayCodes(codes);
            ...
            <Card key={payCode.id}>
              ...
              <CardContent>
                <div className="flex items-center gap-4 text-sm text-muted-foreground">
                  ...
                  {payCode.stackable && <div>Stackable: Yes</div>}
                </div>
              </CardContent>
            </Card>
          ))}
  ...
  return (
    <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
      <TabsList className="grid w-full grid-cols-3">
        <TabsTrigger value="primary">Primary Pay Codes</TabsTrigger>
        <TabsTrigger value="supplemental">Supplemental Pay Codes</TabsTrigger>
        <TabsTrigger value="usage">Usage Report</TabsTrigger>
      </TabsList>

      <TabsContent value="primary" className="space-y-6">
        {renderPayCodesTab(primaryPayCodes, 'Primary Pay Codes', 'Earnings, overtime, and PTO codes')}
      </TabsContent>

      <TabsContent value="supplemental" className="space-y-6">
        {renderPayCodesTab(supplementalPayCodes, 'Supplemental Pay Codes', 'Premiums, banked time, deductions, and benefits')}
      </TabsContent>
